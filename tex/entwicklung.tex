 ...
 \begin{figure}
 	\centering
  	\subfigure[Entgegengesetzter Griff: Die Ausrichtung der Greifer (grau) ist zueinander invertiert. Die Achsen liegen alle parallel, sind jedoch in gegensetzige ausgerichtet. Die Symetrie des Objekts (rosa) und die parallelen Finger der Greifer ermöglichen eine lineare Transjektion entlang der Z-Achsen (grün) ohne Kollisionen mit dem anderen Greifer oder dem Objekt.]{%
 		\includegraphics[width=0.45\textwidth]{fig/grip1}
 		\label{fig:grip1}}
 	\hfill
 	\subfigure[Translation($T_\tau$) an der X-Achse (blau) des Objektes (rose) zur Kollisionsvermeidung der Finger (grau). Die minimale Länge der Translation beträgt die Hälfter der aufsummierten Breite der Finger.]{%
 		\includegraphics[width=0.45\textwidth]{fig/grip3}
 		\label{fig:grip3}}
...
Da diese Funktionalität ein MR-Task ist, werden Nachrichten zwischen den Agenten ausgetauscht, um den Ablauf zu koordinieren. Dieser Nachrichtenaustausch wird in den folgenden Sequenz-Diagrammen dargestellt und analysiert. Dabei werden die drei Arten der Übergabe Geben (Abbildung \ref{fig:gripper1}), Nehmen (Abbildung \ref{fig:gripper2}) und Rendezvous (Abbildung \ref{fig:gripper3}) auf die Anzahl der Nachrichten $k$, sowie deren Komplexität, untersucht. Ein weiterer Aspekt ist das zeitliche Verhalten $t$ und die Unterschiede im Ablauf der drei Varianten.
 ...
 \begin{figure}
 	...
 	\hfill
 	\subfigure[Dieses Sequenzdiagramm zeigt die Rendezvous Variante der Übergabe. Die Aktion kann von beiden Agenten initialisiert werden.]{%
...
\begin{figure}
	\centering
		\subfigure[Testaufbau]{%
			\includegraphics[height=4.5cm]{fig/argostest}
			\label{fig:argostest}}
		\hfill
	\subfigure[Topansicht der Punktwolke, die Farben repräsentieren die Intensität der zurückgeworfenen IR-Strahlen. In der Mitte sind die Umrisse des Objektes erkennbar. Am Rand sind einige abweichende Punkte zu sehen.]{%
		\includegraphics[height=4.5cm]{fig/argostop}
		\label{fig:argostop}}
	\hfill
	\subfigure[Seitenansicht der Punktwolke. Sichtbar sind in der Mitte das Objekt und die kubische Grundform der Punktwolke, die eigentlich der planaren Ebene entsprechen sollte.]{%
		\includegraphics[height=4.5cm]{fig/argoside}
		\label{fig:argoside}}
	\hfill
	\subfigure[Seitenansicht der Punktwolke mit zusätzlicher Stromversorgung am Sensor. Sichtbar ist wieder das Objekt in der Mitte. Die Krümmung der Oberfläche ist schwächer als bei Abbildung \ref{fig:argoside}]{%
		\includegraphics[height=4.5cm]{fig/argospowside}
...
\begin{figure}
	...
	\subfigure[Die XTion Kamera(rot) am Greifer montiert. Das Sichtfeld liegt vor der mobilen Plattform. Durch eine Drehung des Armes kann das Sichtfeld um den Roboter erweitert werden.]{%
		\includegraphics[height=4.5cm]{fig/xtionarm}
		\label{fig:xtionarm}}
		\hfill
		\subfigure[Fotographie von Rose bei der Naherkennung. Der schwarz abgeklebte Teil der Plattform ist gut erkennbar.]{%
			\includegraphics[height=4.5cm]{fig/rosecan}
			\label{fig:xtionarmfot}}
	\hfill
	\subfigure[Sichtfeld der Kamera. Umfasst neben dem Boden auch den vorderen Teil der Plattform. Basierend auf der Nähe zum Arm fehlen unten in der Mitte einige Bildpunkte.]{%
		\includegraphics[height=4.5cm]{fig/ratseye}
...
\begin{figure}[h]
	...
	\subfigure[Mechanismus des Arms im globalen Koordinatensystem.]{%
		\includegraphics[width=0.3\textwidth]{fig/ikeasy1}}
	\hfill
	\subfigure[Neues Koordinatensystem mit Ursprung in $j_2$]{%
		\includegraphics[width=0.3\textwidth]{fig/ikeasy2}}
	\hfill
	\subfigure[Kennzeichnung der zu ermittelnden Winkel. Die Position $\{x_{e}, y_{e}\}$ von $j_4$ wird durch die Offsets von $l_1$,$l_4$ und $l_5$ berechnet.]{%
		\includegraphics[width=0.3\textwidth]{fig/ikeasy3}}
	\label{fig:easyik}
